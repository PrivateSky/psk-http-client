...
function PskHttpClient(remoteEndPoint, agentUid, options){
    ...
    this.startSwarm = function(swarmName, phaseName, ...args){
        ...
        $$.remote.doHttpPost(getRemoteToSendMessage(remoteEndPoint, domainInfo.domain), msgpack.encode(swarm), function(err, res){
            if(err){
                console.log(err);
            }
        });

        return $$.remote.requestManager.createRequest(getRemoteToReceiveMessage(returnRemoteEndPoint, homeSecurityContext), swarm);
    };
