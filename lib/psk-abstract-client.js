...
function PskHttpClient(remoteEndPoint, channelName, options = defaultOptions) {
    ...
    function readyToBeUsed(){
        ...
        if(clientType === PskHttpClient.prototype.PRODUCER_CLIENT_TYPE){
            res = true;
        }
        if(clientType === PskHttpClient.prototype.CONSUMER_CLIENT_TYPE){
    ...
    this.setSenderMode = function () {
        if (typeof clientType !== "undefined") {
            throw new Error(`PskHttpClient is set as ${clientType}`);
        }
        clientType = PskHttpClient.prototype.PRODUCER_CLIENT_TYPE;

        this.sendSwarm = function (swarmSerialization) {
            $$.remote.doHttpPost(getRemoteToSendMessage(remoteEndPoint, channelName));
        };
    };
    ...
    this.setReceiverMode = function () {
        ...
        clientType = PskHttpClient.prototype.CONSUMER_CLIENT_TYPE;
    ...
    function getRemoteToReceiveMessage() {
        return [urlEndWithSlash(remoteEndPoint), urlEndWithSlash(PskHttpClient.prototype.RECEIVE_API_NAME), urlEndWithSlash(encryptChannelName(domain))].join("");
    }

    function getRemoteToSendMessage() {
        return [urlEndWithSlash(remoteEndPoint), urlEndWithSlash(PskHttpClient.prototype.SEND_API_NAME), urlEndWithSlash(encryptChannelName(channelName))].join("");
    }

    function getRemoteToCreateChannel() {
        return [urlEndWithSlash(remoteEndPoint), urlEndWithSlash(PskHttpClient.prototype.CREATE_CHANNEL_API_NAME), urlEndWithSlash(encryptChannelName(channelName))].join("");
    }
}

/********************** constants **********************************/
PskHttpClient.prototype.RECEIVE_API_NAME = "receive-message";
PskHttpClient.prototype.SEND_API_NAME = "send-message";
PskHttpClient.prototype.CREATE_CHANNEL_API_NAME = "create-channel";
PskHttpClient.prototype.PRODUCER_CLIENT_TYPE = "producer";
PskHttpClient.prototype.CONSUMER_CLIENT_TYPE = "consumer";
...
if (typeof $$.remote === "undefined") {
    ...
    function registerPskHttpClient(alias, remoteEndPoint, channelName, options) {
        $$.remote[alias] = new PskHttpClient(remoteEndPoint, channelName, options);
    }

    Object.defineProperty($$.remote, "createRequestManager", {value: createRequestManager});
    Object.defineProperty($$.remote, "registerPskHttpClient", {value: registerPskHttpClient});
